...
const mergeFormValuesIntoInfraEnv = (
  ...
  hostsNetworkConfigurationType: values.hostsNetworkConfigurationType as 'static' | 'dhcp',
});
...
const OptionalConfigurationsStep = () => {
  ...
  const { t } = useTranslation();
  const { setDisconnectedInfraEnv, disconnectedInfraEnv } = useClusterWizardContext();
  ...
  const initialValues = React.useMemo<OptionalConfigurationsFormValues>(
    () =>
      disconnectedInfraEnv ? infraEnvToFormValues(disconnectedInfraEnv) : DEFAULT_INITIAL_VALUES,
  ...
  const onSubmit = React.useCallback(
    ...
    [clearAlerts, addAlert, t, cluster, disconnectedInfraEnv, setDisconnectedInfraEnv],
  );
